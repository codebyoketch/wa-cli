...
- [x] **Phase 15 — Testing**: automated unit tests for pure logic, manual
      verification against a real account for the thin WhatsApp-facing
      layer. Overall coverage measures 37.7% (`go tool
      cover -func=coverage.out`, total across all statements), but that
      number undercounts what's actually covered — see below. The
      non-network core is at or near 80%+: `internal/app`,
      ...
      (~75%), `internal/qr` (100%). `internal/whatsapp` (23.2%) and `cmd`
      (16.0%) have full unit coverage on everything that's actually
      logic — `client.go`'s message classification/forwarding/preview
      helpers and `ingestMessage`; `cmd`'s config field validation,
      JID/message-ref resolution, `--json` handling, shell completion
      generation, and the `captureLibraryStdout` stdout-redirect trick.

      What's left in both packages — `SendText`, `Login`, `Watch`,
      `ListGroups`, `DownloadMedia`, and the `cmd` RunE functions that
      call them — is thin pass-through to a live `*whatsmeow.Client`,
      not logic with its own bugs to catch. Making it unit-testable
      would mean introducing a `waClient` interface for `Client.WA` (so
      a fake can stand in for `*whatsmeow.Client`) — real production-code
      surgery on the type every command and the TUI depends on, with no
      compiler available in the dev sandbox to catch a wrong method
      signature before it ships. Decided deliberately against that
      trade: every one of those call paths has already been exercised
      against a real account, repeatedly, across Phases 2, 4–9, and 8's
      video/audio/voice/document/sticker verification — that's the
      actual testing strategy for this layer, not a stand-in for one.
      Revisit the interface approach only if this grows contributors
      other than the original author, where a regression safety net
      matters more than it does for solo, manually-verified development.
      `internal/notify`'s one-line `beeep.Notify` wrapper and
      `internal/tui` (Bubble Tea UI) are also still 0% — low value for
      unit tests (would either pop real OS notifications during `go
      test` or mostly test a UI framework, not wa-cli's own logic).
      `internal/cli` (0%) is the pre-Cobra hand-rolled router noted as
      dead code in `ARCHITECTURE.md` — candidate for deletion rather
      than tests.
